...
function DropClaimModal({ onClose, onAdd }) {
  ...
  const handleRun = () => {
    setStatus('running');
    setTimeout(() => {
      const newClaim = window.NEW_CLAIM_TEMPLATES[selected];
      onAdd({ ...newClaim, _isNew: true });
      setStatus('done');
      ...
      <div style={{
        ...
        <div style={{ padding: '24px 24px 0' }}>
          ...
          <div style={{ fontSize: '12px', color: '#9CA3AF', marginTop: '4px' }}>
            Live API call to Claude. Takes 3–6 seconds.
          </div>
        </div>
...
// ── Queue Page ────────────────────────────────────────────────────────────────
function QueuePage({ claims, onOpenClaim, onAddClaim }) {
